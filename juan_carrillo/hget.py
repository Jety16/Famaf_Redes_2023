...
def connect_to_server(server_name):
    ...
    sys.stderr.write(f"Contactando al servidor en {ip_address}...\n")

    # Crear socket
    # socket.AF_INET is for IPV4 connections
    # socket.SOCK_STREAM is for TCP connections
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)

    # Juan Carrillo:
    # We need a tuple with the ip_address and the port
    # default port for http: 80

    # http
    server_address = (ip_address, HTTP_PORT)
    if sock.connect(server_address) is None:
        # When connection is succesfully it returns None
        return sock

    raise Exception("Ocurrió un error a la hora de intentar establecer la conexión http")
...
def main():
    ...
    if not url.startswith(PREFIX):
        sys.stderr.write(
            f"La direccion {url} no comienza con {PREFIX}\n")
        sys.exit(1)
