...
def connect_to_server(server_name):
    ...
    # Crear socket
    # COMPLETAR ABAJO DE ESTA LINEA
    # creo un socket que soporta IPv4 y TCP ya que necesito establecer una conexión entre el emisor y el
    # receptor para transmitir datos no puedo solo transmitir paquetes sin  establecer un canal como lo
    # haria UDP
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)

    # Esto pretendia ser lo mismo pero soportando ipv4 y ipv6 pero socket.gethostbyname solo devuelve ipv4
    # asi que no tiene sentido pero si encontrara otra forma de devolver la ip estaria bueno el enfoque
    # info = socket.getaddrinfo(server_name, HTTP_PORT)
    # print(info)
    # ip = info[0][0]
    # parser = str(ip)[str(ip).rfind(".") + 1:]
    # print(parser)
    # if parser == "AF_INET":
    #    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    #    print("ipv4")
    ...
    # creo una tupla con el nombre y puerto del servidor
    addres = (server_name, HTTP_PORT)
    s.connect(addres)
