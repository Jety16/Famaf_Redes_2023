...
"libreria de syscalls calculo :)"
"Libreria que contiene funciones para parsear y analizar strings"
...
def parse_server(url):
    ...
    "path es un string sin el htttp://"
    path = url[len(PREFIX):]
    "path_elements es una lista de los elementos de path, donde cada elemento es separado por un '/'"
    path_elements = path.split('/')
    "El primer elemento de path_elemtents es el server de la URL"
    result = path_elements[0]
    "controlo que el url comienza con prefijo concat el server"
    assert url.startswith(PREFIX + result)
    "controlo que no existe un '/' en el resultado"
...
def connect_to_server(server_name):
    ...
    """
    Es un protocolo TCP (sock_stream) porque HTTP lo requiere, ya que el UDP es un protocolo
    que no espera una conexion para enviar datos. esto puede generar la perdida de datos
    """
...
connect_to_server
    ...
    # encode devuelve el una serie de bytes a partir de un string dado(por default UTF-8 en python)
...
def check_http_response(header):
    ...
    # El primer elemento debe empezar con HTTP/ y el segundo debe ser 80 (son dos elementos)
...
def download(url, filename):
    ...
    # Enviar pedido, recibir respuesta(tambien se descarga los datos a file de output)
    try:
...
# si se llama como main ejecuta el codigo y sale.
